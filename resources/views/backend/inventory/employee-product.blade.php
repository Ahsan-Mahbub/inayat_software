        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="get" action="{{ route('inventory.employee.search') }}">
                        <div class="row mb-3">
                                ...
                                <select class="custom-select select2" name="user_id">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ $user->id == $search_user ? 'selected' : '' }}>
                                            {{ $user->name }} ({{$user->role ? $user->role->role_name : ''}})</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <tbody>
                                ...
                                <?php
                                    ...
                                    if($product)
                                    {
                                        ...
                                        // Get the total purchase return quantity
                                        $purchase_return_qty = App\Models\PurchaseReturn::where('product_id', $purchase->product_id)
                                            ->where('creator_id', $search_user)
                                            ->where('unit_id', $purchase->unit_id)
                                            ->sum('qty');

                                        // Get the total sale quantity
                                        $sale_qty = App\Models\SaleProduct::where('product_id', $purchase->product_id)
                                            ->where('unit_id', $purchase->unit_id)
                                            ->whereHas('sale', function ($query) use ($search_user) {
                                                $query->where('creator_id', $search_user);
                                            })
                                            ->sum('qty');

                                        // Get the total sale return quantity
                                        ...
                                        // Get the total sample request quantity
                                        $sample_request_qty = App\Models\SampleRequestProduct::where('product_id', $purchase->product_id)
                                            ->where('unit_id', $purchase->unit_id)
                                            ->whereHas('sampleRequest', function ($query) use ($search_user) {
                                                $query->where('creator_id', $search_user);
                                            })
                                            ->sum('qty');

                                        // Get the total sample request return quantity
                                        ...
                                        $total_qty = $purchase->total_qty
                                            ...
                                            + $sale_return_qty
                                            - $sample_request_qty
                                            + $sample_request_return_qty;

                                        // Get the last purchase product
                                        $purchase_last_product = App\Models\PurchaseProduct::where('product_id', $purchase->product_id)
                                            ->where('unit_id', $purchase->unit_id)
                                            ->whereHas('purchase', function ($query) use ($search_user) {
                                                $query->where('creator_id', $search_user);
                                            })
                                            ->orderBy('id', 'desc')
                                            ->first();
