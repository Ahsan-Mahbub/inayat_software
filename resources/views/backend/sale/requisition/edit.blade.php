...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <h4 class="mb-0 font-size-18">Update Sale Quotation</h4>
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="/">Dashboard</a></li>
                        <li class="breadcrumb-item">Sale Quotation</li>
                        <li class="breadcrumb-item active">Update Sale Quotation</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <div class="col-12 text-right mb-4">
        ...
        <a href="{{route('sale.requisition.index')}}" class="btn btn-primary btn-sm waves-effect waves-light">Sale Quotation List</a>
        @endisset
    </div>
    <form method="post" action="{{route('sale.requisition.update', $requisition->id)}}">
        @csrf
                        ...
                        <div class="row justify-content-center">
                            <div class="col-md-3 pb-3">
                                <div class="form-floating">
                                    ...
                                    <input type="hidden" class="form-control" name="creator_id" value="{{$requisition->creator_id}}">
                                </div>
                            </div>
                            <div class="col-md-3 pb-3">
                                ...
                                <div class="">
                                    <select class="custom-select select2" name="customer_id" id="customer_id" onchange="getCustomerInfo()" required="">
                                        ...
                                        @if ($customer)
                                            <option value="{{ $customer->id }}" {{$requisition->customer_id == $customer->id ? 'selected' : ''}}>{{ $customer->customer_name }} @if($customer->phone) - {{$customer->phone}} @endif @if($customer->company_name) - {{$customer->company_name}} @endif</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4 pb-3">
                                <div class="form-floating">
                                    ...
                                    <input type="text" readonly id="sub_total" class="form-control" name="subtotal"
                                        required="" value="{{$requisition->subtotal ? $requisition->subtotal : 0}}">
                                </div>
                                ...
                                <div class="row">
                                            ...
                                            <select name="discount" class="form-control" id="discount" onclick="discountValue()">
                                                ...
                                                @if($discount)
                                                    @for ($i = 1; $i <= $discount->discount; $i++)
                                                        <option value="{{ $i }}" {{ $requisition->discount == $i ? 'selected' : '' }}>{{ $i }}</option>
                                                    @endfor
                                                @endif
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-2 m-auto text-center">
                                        <input type="checkbox" class="form-check-input" name="percentage" id="percentage"
                                            @if($requisition->percentage == 1) value="1" checked @else value="0" @endif> %
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4 pb-3">
                                <div class="form-floating">
                                    ...
                                    <input type="text" class="form-control" readonly id="discount_price"
                                        name="discount_price" required="" value="{{$requisition->discount_price ? $requisition->discount_price : 0}}">
                                </div>
                            </div>
                            <div class="col-md-3 pb-3">
                                <div class="form-floating">
                                    ...
                                    <input type="text" class="form-control" name="vat" id="vat"
                                        onkeyup="vatValue()" required="" value="{{$requisition->vat ? $requisition->vat : 0}}">
                                    <input type="hidden" id="vat_price">
                                    <input type="hidden" id="vat_actual_price" name="vat_amount" value="{{$requisition->vat_amount ? $requisition->vat_amount : 0}}">
                                </div>
                            </div>
                            <div class="col-md-3 pb-3">
                                <div class="form-floating">
                                    ...
                                    <input type="text" class="form-control" name="tax" id="tax"
                                        onkeyup="taxValue()" required="" value="{{$requisition->tax ? $requisition->tax : 0}}">
                                    <input type="hidden" id="tax_price">
                                    <input type="hidden" id="tax_actual_price" name="tax_amount" value="{{$requisition->tax_amount}}">
                                </div>
                            </div>
                            <div class="col-md-3 pb-3">
                                <div class="form-floating">
                                    ...
                                    <input type="text" class="form-control" name="ait" id="ait"
                                        onkeyup="aitValue()" required="" value="{{$requisition->ait ? $requisition->ait : 0}}">
                                    <input type="hidden" id="ait_price">
                                    <input type="hidden" id="ait_actual_price" name="ait_amount" value="{{$requisition->ait ? $requisition->ait : 0}}">
                                </div>
                            </div>
                            <div class="col-md-3 pb-3">
                                <div class="form-floating">
                                    ...
                                    <input type="text" readonly class="form-control" name="total_amount"
                                        id="total_price" required="" value="{{$requisition->total_amount}}">
                                </div>
                            </div>
                            <div class="col-md-12 mb-3">
                                <label for="validationCustom04">Terms & Condition : </label> <input type="checkbox" name="show_terms" value="1" @if($requisition->show_terms == 1) checked @endif>
                                <textarea class="form-control editor" name="trams_condition" placeholder="Terms & Condition">{{$requisition->trams_condition}}</textarea>
                            </div>
        ...
        $(document).ready(function() {
            ...
            $(document).on("click", ".addEvenMore", function() {
                ...
                $("body").on("keyup", "." + className, function(e) {
                    ...
                    var searchProductUrl = "{{ route('sale.requisition-search') }}";
                    $.ajax({
                        headers: {
                            ...
                            `
                                $(document).on('click', "." + class_name, function() {
                                        ...
                                        $.ajax({
                                            ...
                                            url: "{{ route('sale-requisition-get-product-details') }}",
                                            type: "POST",
                                            data: {
